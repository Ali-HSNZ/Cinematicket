...
const Menu = () => {
     ...
     return (  
          <nav className="bg-white hidden lg:flex flex-col gap-y-2 h-screen sticky top-0 overflow-y-auto min-w-[300px] max-w-[300px] p-4">
               <Link href={'/dashboard/edit_profile'} className={`w-full py-4 px-2 ${router.asPath.startsWith('/dashboard/edit_profile') ? "bg-red-50 border-red-200" : "hover:bg-red-50 border-transparent hover:border-red-200"} group rounded-md border flex items-center justify-between font-iranyekan-regular text-sm`}>
               ...
               <Link href={'/dashboard/authentication'} className={`w-full py-4 px-2 ${router.asPath.startsWith('/dashboard/authentication') ? "bg-red-50 border-red-200" : "hover:bg-red-50 border-transparent hover:border-red-200"} group rounded-md border flex items-center justify-between font-iranyekan-regular text-sm`}>
                    <div className={`flex items-center gap-x-2 ${router.asPath.startsWith('/dashboard/authentication') ? "text-red-600" : "group-hover:text-red-600 text-gray-700"}  `}>
                         <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5zm6-10.125a1.875 1.875 0 11-3.75 0 1.875 1.875 0 013.75 0zm1.294 6.336a6.721 6.721 0 01-3.17.789 6.721 6.721 0 01-3.168-.789 3.376 3.376 0 016.338 0z" />
                         </svg>
                         احراز هویت
                    </div>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`w-5 h-5 ${router.asPath.startsWith('/dashboard/authentication') ? "text-red-600" : "text-gray-600 group-hover:text-red-600"}`}>
                         <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
                    </svg>
               </Link>
