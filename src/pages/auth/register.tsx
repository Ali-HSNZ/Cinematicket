...
import { authRegister, changeRegisterStatus } from "@slices/auth/register";
...
const AuthRegisterPage = () : JSX.Element => {
     ...
     const {loading , status} = useSelector((state : TRootState) => {return state.authRegister})
     ...
     const validationSchema : object = YUP.object({
          ...
          repeatPass : YUP.string()
               ...
               .test("confirm-repeatPassword" , "رمز عبور مطابقت ندارد.", value => value === formik.values.password),
     })
     ...
     useEffect(() => {
          if(status === EAuthStatus.SUCCESS){
               ...
               dispatch(changeRegisterStatus(EAuthStatus.DEFAULT))
          }
     } , [status])
                         ...
                         <form onSubmit={formik.handleSubmit} className="flex flex-col justify-between items-end gap-x-6 mt-6">
                              <div className="w-full flex flex-col  gap-6">
                                   ...
                                   <div className="w-full relative">
                                        ...
                                        <input type="text" name="repeatPass" onChange={formik.handleChange} onBlur={formik.handleBlur} className={`${formik.errors.repeatPass && formik.touched.repeatPass ? "bg-red-50 border-red-200 focus:border-red-300 hover:border-red-300" : "bg-gray-50 border-gray-200 hover:border-gray-300 focus:border-gray-300"} mt-2  placeholder:text-gray-500 text-gray-600  py-3 pl-2 pr-9 rounded-md font-iranyekan-regular text-sm border   focus:outline-none w-full`} />
                                        {formik.errors.repeatPass && formik.touched.repeatPass && <p className="font-iranyekan-regular text-xs mt-2 text-red-600">{formik.errors.repeatPass}</p>}
                                   
                                   </div>
                              </div>
                              <Link href={'/auth/login'} className="font-iranyekan-regular text-sm text-red-600 text-right w-full underline underline-offset-2 mt-4">حساب کاربری دارید؟</Link>
                              <div className="w-full h-auto mt-4  flex items-center justify-end gap-x-4">
                                   <button disabled={!formik.isValid || loading} type="submit" className="group flex justify-center w-[110px] disabled:bg-gray-600  disabled:cursor-not-allowed disabled:text-gray-200  bg-red-500 hover:bg-red-600 duration-150 rounded-md   py-3 h-fit">
                                        {loading ? (
                                             ...
                                             <p className="group-disabled:text-gray-100 text-sm font-iranyekan-bold text-red-100">ثبت نام</p>
                                        )}
                                   </button>
                              </div>
                         </form>
